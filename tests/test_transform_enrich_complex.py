...
def test_enrich_substance_parsing_complex(base_epar_df: pl.DataFrame) -> None:
    """
    Complex Case: Substance string with mixed delimiters (+ and /) and whitespace.
    """
...
def test_enrich_atc_validation_filtering(base_epar_df: pl.DataFrame) -> None:
    """
    Complex Case: ATC codes containing invalid formats.
    ...
    # B02BD02 = Valid
    input_str = "A01BC01, XYZ, A01BC012, B02BD02"
    ...
    atc_list = result["atc_code_list"].item()
    assert "A01BC01" in atc_list
    assert "B02BD02" in atc_list
    assert "XYZ" not in atc_list
    assert "A01BC012" not in atc_list
    assert len(atc_list) == 2
