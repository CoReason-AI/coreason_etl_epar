...
def create_gold_layer(silver_df: pl.DataFrame) -> Dict[str, pl.DataFrame]:
    ...
    df = silver_df.with_columns(
        ...
        .map_elements(generate_coreason_id, return_dtype=pl.String)
        .alias("coreason_id")
    )
    ...
    # ATC
    atc = current_df.select(["coreason_id", "atc_code_list"]).explode("atc_code_list").drop_nulls().select([
        "coreason_id",
        pl.lit("ATC_CODE").alias("feature_type"),
        pl.col("atc_code_list").alias("feature_value")
    ])

    # Substance
    substance = current_df.select(["coreason_id", "active_substance_list"]).explode("active_substance_list").drop_nulls().select([
        "coreason_id",
    ...
    area = current_df.select(["coreason_id", "therapeutic_area"]).drop_nulls().with_columns(
        pl.col("therapeutic_area").str.split(";").alias("area_list")
    ).explode("area_list").select([
        "coreason_id",
    ...
    return {
        ...
        "fact_regulatory_history": fact_history,
        "bridge_medicine_features": bridge
    }
