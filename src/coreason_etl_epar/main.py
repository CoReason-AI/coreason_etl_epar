...
def run_pipeline(
    ...
    with logger.contextualize(ingestion_batch_id=ingestion_batch_id):
        ...
        if current_history is None or current_history.is_empty():
            ...
            hash_cols = ["status", "spor_mah_id", "is_orphan"]
            id_col = "coreason_id"

            cols_for_history = ["coreason_id", "status", "spor_mah_id", "is_orphan"]

            # Map orphan to is_orphan before history alignment if present
            if "orphan" in new_history_snapshot.columns:
                new_history_snapshot = new_history_snapshot.with_columns(
                    pl.col("orphan").fill_null(False).alias("is_orphan")
                )
            else:  # pragma: no cover
                new_history_snapshot = new_history_snapshot.with_columns(
                    pl.lit(False, dtype=pl.Boolean).alias("is_orphan")
                )
